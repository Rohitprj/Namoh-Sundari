...
const CheckoutPage = ({navigation}) => {
  ...
  useEffect(() => {
    fetchCartData();
    fetchAddressData();
  }, [isFocused, fetchCartData, fetchAddressData]);
...
// const CURRENT_USER_ID = 'your_actual_user_id_here'; // This needs to be dynamically fetched or passed
...
//   // Define currencyConfig directly or fetch if dynamic
//   const [currencyConfig] = useState({
//     applicationData: {
//       currency: 'INR',
//       // !! IMPORTANT: Replace with your actual LIVE or TEST Razorpay Key ID !!
//       // For testing, use a test key (starts with 'rzp_test_...')
//       // For production, use your live key (starts with 'rzp_live_...')
//       razorpayKeyId: 'rzp_test_YOUR_TEST_KEY_ID_HERE', // <-- CHANGE THIS
//     },
//   });
...
//         // Handle error, e.g., navigate to login or show an alert
//       }
//     };
//     fetchUser();
//   }, []);
...
//   const fetchCartData = useCallback(async (showLoading = true) => {
//     // const userId = await getUserData(); // This is now handled by currentUser state
//     // console.log(`User ID for order: ${JSON.stringify(userId.userId, null, 2)}`);
...
//       !currencyConfig.applicationData.razorpayKeyId ||
//       currencyConfig.applicationData.razorpayKeyId.includes(
//         'YOUR_TEST_KEY_ID_HERE', // Updated to match the placeholder
//       )
//     ) {
//       Alert.alert(
//         'Configuration Error',
//         'Please set your actual Razorpay Key ID in currencyConfig.applicationData.razorpayKeyId.',
//       );
//       return false;
//     }

//     if (!deliveryAddress?._id) {
//       Alert.alert('Error', 'Delivery address is missing for payment.');
//       return false;
//     }

//     if (!currentUser?.userId) {
//       Alert.alert('Error', 'User ID is missing for payment.');
//       return false;
//     }

//     try {
//       // 2. Create Razorpay order on your backend
//       // This step generates a Razorpay Order ID which is crucial for the payment gateway.
//       const createOrderResponse = await axiosInstance.post(
//         '/web/create-razorpay-order',
//         {
//           amount: Math.round(Number(amount)), // Amount should be in the smallest currency unit (e.g., paise for INR)
//           currency: currencyConfig.applicationData.currency || 'INR',
//           addressId: deliveryAddress._id,
//           // productId: firstProductId, // Removed as it might not be relevant for a cart-level order
//           // The backend should derive product details from the cart for the order
//           userId: currentUser.userId, // Use dynamically fetched user ID
//         },
//       );
//       const res = createOrderResponse.data;

//       const orderId = res?.orderId; // This is the Razorpay order_id from your backend
//       const amountInPaise = Math.round(Number(amount) * 100); // Razorpay expects amount in paise
...
//         amount: amountInPaise,
//         name: 'Shopinger E-Commerce',
//         order_id: orderId, // Crucial: The order ID from your backend
//         prefill: {
//           // It's highly recommended to use actual user data here
//           email: currentUser.email || 'customer@example.com', // Replace with actual user email
//           contact: currentUser.mobile || '9876543210', // Replace with actual user contact
//           name: currentUser.name || 'John Doe', // Replace with actual user name
//         },
//         theme: {color: '#ff6600'},
//       };
...
//           orderId: rzpOrderId, // Pass Razorpay order ID
//         });
//         await fetchCartData(false); // Clear the cart after successful payment
//         return true; // Indicate success
//       } else {
//         Alert.alert(
//           'Payment Verification Failed',
...
//       if (error.code === RazorpayCheckout.external.ErrorCode.DISMISS) {
...
//   const handlePlaceOrder = async () => {
//     // Ensure currentUser and deliveryAddress are loaded
...
//           paymentOrderId: `COD-${Date.now()}`, // Generate a unique ID for COD orders
//           orderStatus: 'PENDING',
//           addressId: deliveryAddress._id, // Use _id from the fetched address
//           totalAmount: cartSummary.totalAmount,
//           userId: currentUser.userId, // Use dynamically fetched user ID
//           notes: 'Thank you for your order!',
//         };
...
//           navigation.navigate('OrderSuccessScreen', {
//             paymentMode: 'COD',
//             orderId: response.data.orderId, // Pass backend order ID if returned
//           });
//         } else {
//           Alert.alert('Order Failed', response.data.message || 'Failed to place COD order. Please try again.');
//         }
...
//         if (!paymentSuccess) {
//           console.log('Razorpay payment did not complete successfully.');
//           // You might want to keep the payment screen open or show a specific message
//         }
//       }
//     } catch (error) {
//       console.error('Error placing order:', error);
...
//   if (loadingCart || loadingAddress || !currentUser) { // Added currentUser to loading check
//     return (
//       <SafeAreaView style={styles.safeArea}>
//         <View style={styles.loadingContainer}>
//           <ActivityIndicator size="large" color="#ff6600" />
...
//             cartItemsToDisplay.length === 0 ||
//             !deliveryAddress ||
//             !currentUser // Disable if user data isn't loaded
//           }>
//           {isPaying ? (
//             <ActivityIndicator color="#fff" size="small" />
//           ) : (
